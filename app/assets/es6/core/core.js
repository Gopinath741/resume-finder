...
export default class Core {
  ...
  setLogo() {
    if (
      $(sideNav).hasClass(navMenuCollapse) &&
      !$(sideNav).hasClass(navMenuQuickExpand)
    ) {
      $(`${sideNav} .logo img`).attr(
        'src',
        `/assets/images/logo/${
          $(sideNav).hasClass('nav-menu-dark') ? 'logo-fold-white' : 'logo-fold'
        }.png`
      );
    } else {
      $(`${sideNav} .logo img`).attr(
        'src',
        `/assets/images/logo/${
          $(sideNav).hasClass('nav-menu-dark') ? 'logo-white' : 'logo'
        }.png`
      );
    }
  }
  ...
  sideNav() {
    const active = 'active';
    const drodpDownItem =
      '.vertical-menu .nav-menu .nav-submenu .nav-menu .nav-menu-item';

    if ($(drodpDownItem).hasClass(active)) {
      $(drodpDownItem + '.' + active)
        .parent()
        .parent()
        .addClass('open');
    }
