const searchResumeButton = document.getElementById("resume-search-submit");
...
searchResumeButton.addEventListener("click", (e) => {
    ...
    } else if(document.getElementById('certificationOption2').checked) {
        input_json['certification'] = 'No'
    }
  
    $.ajax({
        url: 'http://127.0.0.1:5000/search_resume',
        method: 'POST',
        // headers: {
        ...
        success: (add_resume_resp) => {
          console.log('search Resume Success')
          console.log(add_resume_resp)
          if (add_resume_resp['status'] == 200) {
            alert('Found  Resume')
            // window.location.href = "http://127.0.0.1:4200/list-resume.html";
          } else {
